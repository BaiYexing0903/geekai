...
func TestClientCreateAssetParsesWrappedResponse(t *testing.T) {
	calls := []string{}
	client := &Client{
		config: types.SeedanceConfig{ApiURL: "https://seedance.example.com", BearerToken: "token"},
		httpClient: &http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
			...
			case "/open/CreateAsset":
				body = `{"code":0,"data":{"Id":"asset-wrapped"}}`
			case "/open/GetAsset":
				body = `{"code":0,"data":{"Id":"asset-wrapped","Name":"测试人像","URL":"https://cdn.example.com/person.jpg","AssetType":"Image","Status":"Active"}}`
			default:
	...
	if got.ID != "asset-wrapped" || got.URL != "https://cdn.example.com/person.jpg" || got.Name != "测试人像" {
